...
- Two shared assets, included by pages via **relative URLs**:
  ...
  - **`starstuff.js`** — shared front-end behavior for paged zines: **the pager itself**
    (`changePage()`, the prev/next buttons, the arrow keys — added 2026-09-13, see *Page
    conventions*), the per-spread footer prev/next controls, and `#spread-N` deep linking. It is a
    safe no-op on pages that aren't paged zines: each feature guards on the markup it needs, so a
    page with no `.spread`, or spreads but no pager buttons, gets nothing.
...
- **The CSP is generated by `tools/build-csp.mjs`, and `--check` is a ship
  ...
  produce a nonce; the third option, and the one this repo took, is **hashes**. There are **34
  distinct inline `<script>` bodies** (across 58 elements, policy 2,197 bytes — it was 50 across 163
  and 3,061 bytes until 2026-09-13, when moving the pager into `starstuff.js` retired 17 variants at
  a stroke: **duplicated inline script is duplicated CSP surface**) and **6 distinct inline event
...
- Paged zines: include `<script src="starstuff.js"></script>` and structure spreads as `.spread`
  (with a `.spread.active`), each with a `.spread-footer` containing a `.spread-footer-right` page
  counter. IDs run `spread-1..N` in document order. Give the controls `id="prev-btn"`,
  `id="next-btn"` and `id="page-counter"`.
- **DON'T WRITE A `changePage()` — `starstuff.js` provides it (2026-09-13).** This bullet used to
  say *expose a global `changePage(dir)`*, and 107 pages did: **17 variants, 115,101 bytes, and the
  only thing that differed between them was `const total = N`** — the spread count, which the shared
  file already computes as `spreadCount()`. On all 107 the declared total matched the DOM exactly, so
  it was a constant the page could not get wrong without the copy being wrong too. The shared pager
  reads position from the DOM, paints the counter and buttons, and binds the arrow keys.
  **A page that defines its own `window.changePage` keeps it** — `installPager()` returns early —
  so special paging is still possible; say why on the page if you do it.
  **`shorthand-evolution.html` is now the only page with its own**, because it deliberately does not
  load `starstuff.js`, and it is therefore also the only paged zine with no `#spread-N` deep linking.
  **The guard is `#prev-btn` AND `#next-btn` AND at least one `.spread`** — the two scroll zines have
  spreads and no buttons, so they get no pager, which is what makes the form work unchanged.
