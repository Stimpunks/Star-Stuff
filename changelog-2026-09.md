...
**Three corrections made before publishing rather than after.** A first draft of the penguin item called this a third case from a third kingdom, closing the cryptic-diversity seed opened on 18 September; penguins and frogs are the same kingdom, so it is a third case and still a second kingdom, and the page says that. A first draft of the gravity item treated the sealed envelope as the paper’s; it is the release’s, the Crossref deposit says nothing about blinding, and the entry now marks it. **And a first draft dated the *P. poncetii* description as “a couple of years ago” without opening the reference** — it is Tyler *et al.*, *Ecology and Evolution* **10**, 13836 (2020), six years, and reading the entry turned up the better fact underneath it: that paper is itself titled *Morphometric and genetic evidence for four species of gentoo penguin*, so the disagreement is not about the number at all. **One arithmetic statement is marked as ours and not the abstract’s**: the reported disagreement of 2.5 × 10^−4^ against a stated uncertainty of 5.7 × 10^−5^ is roughly four times the uncertainty, which is our division of the abstract’s two numbers.

**A generator may not read a partial clone, either.** This edition was built in a checkout with **51 commits of history** rather than 460, and `build-derived.mjs` derives every publication date from `git log --diff-filter=A` — so for every page older than the shallow boundary it returned the boundary commit’s date instead of the real one. [What’s New](https://starstuff.earth/whats-new.html) regenerated with **5 day headings instead of 42**, dating 170 pieces to September that were published in July and August, and `feed.xml` rewrote its dates to match. **Every gate passed it**, and each was right to: the markup is valid, the classes are live, the contrast is fine, and `check-derived.mjs` compares each generator against *what that generator would write today*, which in a shallow clone is exactly the wrong file. The tell was `git diff --stat`: 1,463 changed lines in a page where adding one piece should change about ten. Fixed by `git fetch --unshallow` and regenerating; the corrected diff is 20 lines. The general rule [recorded on 20 September](#2026-09-20-feed-timezone) — a generator may not read the machine it runs on — now has a sibling: *it may not read an incomplete copy of the repository either, and a shallow clone is the one machine difference that looks like data.*
